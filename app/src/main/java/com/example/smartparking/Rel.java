...
public class Rel extends AppCompatActivity {
    private TextView pname, sln, cname, cemail,cnumber, cdate, centry, cexit, cvt, cvn, cpay, cinvoice, cdt,slnret;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pdfview = findViewById(R.id.pdfviewre);
        builder = new AlertDialog.Builder(Rel.this);
        paycon=findViewById(R.id.payconre);
        cdt=findViewById(R.id.cdtre);
        slnret=findViewById(R.id.slnret);
        dnbutton=findViewById(R.id.dnbuttonre);
        cname=findViewById(R.id.cnamere);
        cemail=findViewById(R.id.cemailre);
        cnumber=findViewById(R.id.cnumberre);
        cdate=findViewById(R.id.cdatere);
        centry=findViewById(R.id.centryre);
        cexit=findViewById(R.id.cexitre);
        cvt=findViewById(R.id.cvtre);
        cvn=findViewById(R.id.cvnre);
        cpay=findViewById(R.id.cpayre);
        cinvoice=findViewById(R.id.cinvoicere);
        Intent intent = getIntent();
        String invoice = intent.getStringExtra("inv");
        pname=findViewById(R.id.pnamere);
        String date = intent.getStringExtra("date");
        pname.setText("Srinagar");
        sln=findViewById(R.id.slnre);

        FirebaseDatabase.getInstance().getReference("booking").child("Srinagar").child(date).child(invoice).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if(book!=null)
                {
                    ...
                    FirebaseDatabase.getInstance().getReference("booking").child("Srinagar").child(date).child(invoice).child("Relocated").addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            String relt=snapshot.getValue(String.class);
                            slnret.setText(relt);
                        }
                        ...
                        FirebaseDatabase.getInstance().getReference("Users").child(uiid).addListenerForSingleValueEvent(new ValueEventListener() {
                            ...
                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                ...
                                if(pd!=null){
                                    ...
                                    Toast.makeText(Rel.this, "Something wrong happened", Toast.LENGTH_SHORT).show();
                                }
                            }

                            @Override
                            public void onCancelled(@NonNull DatabaseError error) {
                                Toast.makeText(Rel.this, error.getMessage(), Toast.LENGTH_SHORT).show();
                            }
                        });
                    }else{
                        Toast.makeText(Rel.this, "Slot booked by another user", Toast.LENGTH_SHORT).show();
                    }
    ...
    void requestPermission(){
        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.R){
            ...
            ActivityCompat.requestPermissions(Rel.this,permission,30);
        }
    }
    ...
    private void invoicealert() {
        ...
        builder.setNegativeButton(Html.fromHtml("<font color='#8f0404'> Continue </font>"), new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialogInterface, int i) {
                Intent intent = new Intent(Rel.this, loggedinn.class);
